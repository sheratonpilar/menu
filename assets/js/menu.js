...
var UI={es:{buscar:'Buscar un plato…',sin:'No encontramos nada con eso.',act:'Actualizado',ver:'Ver la carta de vinos'},
en:{buscar:'Search a dish…',sin:'Nothing matches that.',act:'Updated',ver:'See the wine list'},
pt:{buscar:'Buscar um prato…',sin:'Nada encontrado.',act:'Atualizado',ver:'Ver a carta de vinhos'}};
...
bar.innerHTML='<div class="bar__top">'+(c.logo?'<img class="bar__logo" src="'+BASE+'assets/img/logos/'+esc(c.logo)+'" alt="">':'')+
...
'</div><div class="search"><input id="q" type="search" autocomplete="off" placeholder="'+esc(UI[lang].buscar)+'"></div>'+
...
paint(data.secciones);spy();}
function paint(secciones){
var fecha=new Date(data.carta.actualizado).toLocaleDateString(lang==='en'?'en-GB':lang==='pt'?'pt-BR':'es-AR');
var html='<div class="wrap">';
if(!secciones.length){app.innerHTML=html+'<p class="empty">'+esc(UI[lang].sin)+'</p></div>';return;}
...
html+='<footer class="foot">'+(vt?'<p><a href="'+BASE+'carta/'+esc(vt)+'/">'+esc(UI[lang].ver)+' &rarr;</a></p>':'')+
(t(data.carta.pie)?'<p>'+esc(t(data.carta.pie))+'</p>':'')+'<p>'+esc(UI[lang].act)+': '+fecha+'</p></footer></div>';
app.innerHTML=html;}
